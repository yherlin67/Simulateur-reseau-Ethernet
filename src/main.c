...
uint32_t convertIP(const char *ipStr) {
...
uint64_t convertMAC(const char *macStr) {
...
void displayBinaryIP(uint32_t ip) {
    for (int i = 0; i < 32; i++) {
...
void displayBinaryMAC(uint64_t mac) {
    for (int i = 0; i < 48; i++) {
...
void ReadFile(const char *pathFile, struct reseau *res) {
